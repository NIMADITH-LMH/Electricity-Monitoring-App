...
class UsageNotificationTestScreen extends StatefulWidget {
  ...
  @override
  State<UsageNotificationTestScreen> createState() =>
      _UsageNotificationTestScreenState();
}

class _UsageNotificationTestScreenState
    extends State<UsageNotificationTestScreen> {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 16),
                      Text('Budget: 100 kWh (Fixed for testing)'),
                      SizedBox(height: 16),
                            ...
                            child: Slider(
                              ...
                              onChanged: (value) {
                                setState(() {
                                  ...
                                  _usageController.text = _selectedUsage
                                      .toString();
                                });
                              },
                            ),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: Text('Test Background Task'),
                        ),
                      ),
                    ],
                  ),
  ...
  void _triggerUsageUpdate() {
    ...
    notifier
        .updateUsage(userId, _selectedUsage)
        .then((_) {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        await FirebaseFirestore.instance
            .collection('users')
            .doc(currentUser.uid)
            .update({'lastNotified': 0, 'lastNotifiedDate': ''});
  ...
  void _testBackgroundTask() async {
    try {
      // Create a notifier instance to test the background functionality
      final notifier = UsageNotifier();

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Running background task test...'),
          duration: Duration(seconds: 2),
        ),
      );

      // Test the checkAllUsers method that's used in background tasks
      await notifier.checkAllUsers();

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Background task test completed successfully'),
